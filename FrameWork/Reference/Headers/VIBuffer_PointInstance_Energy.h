...
class ENGINE_DLL CVIBuffer_PointInstance_Energy final : public CVIBuffer
{
...
protected:
	explicit CVIBuffer_PointInstance_Energy(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CVIBuffer_PointInstance_Energy(const CVIBuffer_PointInstance_Energy& rhs);
	virtual ~CVIBuffer_PointInstance_Energy() = default;
public:
...
public:
	static CVIBuffer_PointInstance_Energy* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext/*, const _tchar* pShaderFilePath, _uint iNumInstance*/);
