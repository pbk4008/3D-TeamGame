...
CEffect_Energy::CEffect_Energy()
{
}

CEffect_Energy::CEffect_Energy(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
...
HRESULT CEffect_Energy::NativeConstruct_Prototype()
{
...
HRESULT CEffect_Energy::NativeConstruct(const _uint _iSceneID, void* pArg)
{
	...
	if (nullptr != pArg)
	{
		memcpy(&m_Desc, pArg, sizeof(EF_PAR_ENERGY_DESC));
	}
...
_int CEffect_Energy::Tick(_double TimeDelta)
{
...
_int CEffect_Energy::LateTick(_double TimeDelta)
{
...
CEffect* CEffect_Energy::Copy()
{
	CEffect_Energy* pEffect = new CEffect_Energy(m_pDevice, m_pDeviceContext);
	if (FAILED(pEffect->NativeConstruct_Prototype()))
	{
		MSGBOX("Energy Copy Fail");
...
void CEffect_Energy::Set_Reset(_bool bReset)
{
...
HRESULT CEffect_Energy::SetUp_Components()
{
	...
	if (FAILED(__super::SetUp_Components((_uint)SCENEID::SCENE_STATIC, L"Proto_Component_VIBuffer_PointInstance_Energy", L"Com_VIBuffer", (CComponent**)&m_pBuffer, &m_backupDesc)))
		return E_FAIL;

	return S_OK;
}

CEffect_Energy* CEffect_Energy::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	/* 원형객체 생성할때 초기화 */
	CEffect_Energy* pInstance = new CEffect_Energy(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CEffect_Energy");
...
CGameObject* CEffect_Energy::Clone(const _uint _iSceneID, void* pArg)
{
	/* 복제본 생성할때는 아래함수 호출해서 추가 초기화를 진행 */
	CEffect_Energy* pInstance = new CEffect_Energy(*this);
	if (FAILED(pInstance->NativeConstruct(_iSceneID, pArg)))
	{
		MSGBOX("Failed to Creating Clone CEffect_Energy");
		Safe_Release(pInstance);
	}
