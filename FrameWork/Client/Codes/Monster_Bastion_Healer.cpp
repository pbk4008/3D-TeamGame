...
HRESULT CMonster_Bastion_Healer::NativeConstruct(void* _pArg)
{
...
HRESULT CMonster_Bastion_Healer::Ready_Components()
{
	...
	_float4 vPosition = { 3.f, 0.f, 6.f, 1.f };
	m_pTransform->Set_State(CTransform::STATE_POSITION, XMLoadFloat4(&vPosition));

	if (FAILED(SetUp_Components((_uint)SCENEID::SCENE_TEST_JS, L"Model_Bastion_Healer", L"Model", (CComponent**)&m_pModel)))
		return E_FAIL;
	m_pModel->Add_Material(g_pGameInstance->Get_Material(L"BastionTierII_Top"), 0);
	m_pModel->Add_Material(g_pGameInstance->Get_Material(L"BastionTierII_Down"), 1);
	m_pModel->Add_Material(g_pGameInstance->Get_Material(L"BastionTierII_Fur"), 2);
...
CGameObject* CMonster_Bastion_Healer::Clone(void* _pArg)
{
